...
class Client():
    ...
    def __init__(self, mode: str) -> None:
        ...
        if mode == 'REAL':
            ...
            raise ValueError("Invalid mode argument. Must be either 'REAL' or 'DEMO'.")


        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket_connection = ssl.wrap_socket(sock, ssl_version = ssl.PROTOCOL_TLS)

        sock_stream = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket_stream_connection = ssl.wrap_socket(sock_stream, 
    ...
    def connect(self):
        ...
        """
        try:
            self.socket_connection.connect((self.user.host, self.user.main_port))
    ...
    def connect_stream(self):
        ...
        """
        try:
            self.socket_stream_connection.connect((self.user.host, self.user.streaming_port))
            logging.info(f'Connected successfully to streaming port on {self.user.host}')
            self.connection_stream = True
        except Exception as e:
            logging.error(f'Not connected to streaming port on {self.user.host}, {e}')
    ...
    def stream_send(self, message:dict) -> int:
        ...
        """
        return self.socket_stream_connection.send(json.dumps(message).encode('utf-8'))
